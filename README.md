...
キャラクターは`characterlist.json`にて定義されています。本ファイルを編集することで、キャラクターの追加・削除ができます。

`id`キーは一意である必要があります（文字列であれば色名でなくとも構いません）。また、当該キャラクターを呼び出すには、`run`の第一引数にこの`id`の値を入れることになります。

`hello`キーと`bye`キーは、キャラクターの呼び出し時および本プログラムの終了時の台詞です。LLMが応答を返す部分の台詞と文体などが一致するよう、実際に使用する予定のLLMで生成するのがおすすめです。
...
## 今後の構想
1. 標準出力を踏まえた応答 
2. 長い応答モード（特定コマンドでmax-tokensを引き上げる）
3. その他設定をいじれるconfigファイルの作成・対応
